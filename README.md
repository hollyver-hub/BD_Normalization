...
| **Gols amontoados:** Todos os gols e eventos de um jogador ficavam aglomerados em um único texto na mesma linha (ex: `G40' G87'`). | **Linha por Gol:** Criamos a tabela fato de eventos `tb_goals`. Agora, cada gol do campeonato gera uma linha exclusiva no banco. | **1ª Forma Normal (Eliminação de Grupos Repetidos):** Permite realizar contagens e agregações matemáticas diretamente via SQL (como `COUNT` e `SUM`). |
